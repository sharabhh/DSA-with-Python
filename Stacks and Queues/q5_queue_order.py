# a) You are given a class MyQueue which implements a queue using two stacks. Your task is to implement the enqueue method which should add an element to the back of the queue.
...
# b) You have been tasked with implementing a queue data structure using two stacks in Python, and you need to write the dequeue method.
...
class Stack:
    ...
    def dequeue(self):
        ...
        while not self.is_empty():
            new_stack.append_stack(self.pop_stack())
        popped_element = new_stack.pop_stack()
        while not new_stack.is_empty():
            self.append_stack(new_stack.pop_stack())
